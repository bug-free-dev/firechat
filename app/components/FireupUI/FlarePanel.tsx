...
export default function FlarePanel() {
	const features = [
		{
			icon: <FiGift className="text-yellow-500/70 w-6 h-6" />,
			title: 'Kudos',
			desc: 'Earn & spend playful perks',
		},
		{
			icon: <FiUsers className="text-orange-500/70 w-6 h-6" />,
			title: 'Invite-only',
			desc: 'Join rooms using secret codes',
		},
		{
			icon: <FiStar className="text-pink-500/70 w-6 h-6" />,
			title: 'Fun & Safe',
			desc: 'Playful rules, zero toxicity',
		},
		{
			icon: <FiLock className="text-purple-500/70 w-6 h-6" />,
			title: 'Identifier',
